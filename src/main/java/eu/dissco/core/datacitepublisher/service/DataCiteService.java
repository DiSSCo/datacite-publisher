...
public abstract class DataCiteService {
	...
	protected DcRequest buildDcRequest(DigitalSpecimen digitalSpecimen) {
		return buildDcRequest(digitalSpecimen.get10320Loc(), properties.getLandingPageSpecimen(), SPECIMEN_ALT_ID_TYPE,
				digitalSpecimen.getNormalisedPrimarySpecimenObjectId(), digitalSpecimen.getSpecimenHostName(),
				digitalSpecimen.getSpecimenHost(), digitalSpecimen.getPidRecordIssueDate(), digitalSpecimen.getPid(),
				digitalSpecimen.getReferentName(), SPECIMEN_TYPE, getDescriptionForSpecimen(digitalSpecimen),
				getSubjectsForSpecimen(digitalSpecimen), getRights());
	}
	...
	protected DcRequest buildDcRequest(DigitalMedia mediaObject) {
		return buildDcRequest(mediaObject.get10320Loc(), properties.getLandingPageMedia(), MEDIA_ALT_ID_TYPE,
				mediaObject.getPrimaryMediaId(), mediaObject.getMediaHostName(), mediaObject.getMediaHost(),
				mediaObject.getPidRecordIssueDate(), mediaObject.getPid(), mediaObject.getReferentName(), MEDIA_TYPE,
				getDescriptionForMedia(mediaObject), getSubjectsForMedia(mediaObject), getRights());
	}

	protected DcRequest buildDataCiteTombstoneRequest(DcAttributes dcAttributes, TombstoneEvent event) {
		var description = new ArrayList<>(dcAttributes.getDescriptions());
		description.add(DcDescription.builder().description("This DOI has been tombstoned").build());
		...
		dates.add(DcDate.builder().date(DATACITE_FORMATTER.format(Instant.now())).dateType("Withdrawn").build());
	...
	private DcRequest buildDcRequest(String xmlLoc, String landingPage, String altIdType, String localId,
			String hostName, String hostId, String pidRecordIssueDate, String pid, String referentName, String dcType,
			List<DcDescription> descriptions, List<DcSubject> subjects, List<DcRights> rights) {
		try {
			var xmlLocs = xmlLocReader.getLocationsFromXml(xmlLoc);
			var url = xmlLocs.isEmpty() ? null : XmlLocReader.getLandingPageLocation(xmlLocs, landingPage);
			var issueDate = getDate(pidRecordIssueDate);
			return DcRequest.builder()
				.data(DcData.builder()
					.attributes(DcAttributes.builder()
						.alternateIdentifiers(getAltIds(altIdType, localId))
						.contributors(getContributors(hostName, hostId))
						.creators(getCreator(hostName, hostId))
						.dates(getDates(issueDate))
						.descriptions(descriptions)
						.doi(getDoi(pid))
						.publicationYear(getPublicationYear(issueDate))
	...
	private List<DcAlternateIdentifier> getAltIds(String idType, String altId) {
		...
		return List
			.of(DcAlternateIdentifier.builder().alternateIdentifier(altId).alternateIdentifierType(idType).build());
	}
	...
	private List<DcContributor> getContributors(String hostName, String hostId) {
		...
		return List.of(DcContributor.builder().name(hostName).nameIdentifiers(getNameIdentifiers(hostId)).build());
	}
	...
	private List<DcDate> getDates(ZonedDateTime pidIssueDate) {
		...
		return List.of(DcDate.builder().date(pidIssueDate.format(DATACITE_FORMATTER)).dateType("Issued").build());
	}
	...
	private List<DcDescription> getDescriptionForSpecimen(DigitalSpecimen digitalSpecimen) {
		...
		if (digitalSpecimen.getSpecimenHostName() != null) {
			descriptionList.add(DcDescription.builder()
				.description("Digital Specimen for the physical specimen hosted at "
						+ digitalSpecimen.getSpecimenHostName() + ".")
				.build());
		}
		if (digitalSpecimen.getMaterialSampleType() != null) {
			descriptionList.add(DcDescription.builder()
				.description("Material sample type is " + digitalSpecimen.getMaterialSampleType() + ".")
				.build());
		}
	...
	private List<DcSubject> getSubjectsForMedia(DigitalMedia mediaObject) {
		...
		if (mediaObject.getMimeType() != null) {
			subjectList
				.add(DcSubject.builder().subjectScheme("mediaFormat").subject(mediaObject.getMimeType()).build());
		}
		if (mediaObject.getLinkedDigitalObjectType() != null) {
			subjectList.add(DcSubject.builder()
	...
	private List<DcTitle> getTitles(String referentName) {
		...
		return List.of(DcTitle.builder().title(referentName).build());
	}

	private List<DcRelatedIdentifiers> getRelatedIdentifiers(List<String> xmlLocations, String landingPage) {
	...
	private DcType getDcType(String type) {
		var resourceTypeGeneral = type.equals(SPECIMEN_TYPE) ? RESOURCE_TYPE_GENERAL_DATASET
				: RESOURCE_TYPE_GENERAL_IMAGE;
		return DcType.builder().resourceType(type).resourceTypeGeneral(resourceTypeGeneral).build();
	}
