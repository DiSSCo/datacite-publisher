...
public class WebClientErrorHandling {
	...
	public static Mono<ClientResponse> exchangeFilterResponseProcessorDoi(ClientResponse response) {
		...
		if (status.is4xxClientError() || status.is5xxServerError()) {
			return response.bodyToMono(JsonNode.class).flatMap(body -> {
				log.error("An error has occurred with the DOI service. Status code: {}, response: {}", status, body);
	...
	public static Mono<ClientResponse> exchangeFilterResponseProcessorDataCite(ClientResponse response) {
		var status = response.statusCode();
		if (status.is4xxClientError() || status.is5xxServerError()) {
